...
import com.msu.DTO.GetTimeSlotIterativeDTO;
...
import com.msu.DTO.TimeSlotDTO;
...
public class SectionScheduleServiceImpl implements SectionScheduleService {
	...
	public List<ProfessorDropDownListResponseDTO> professorDropDownList(
		...
		List<CourseProfessorMapping> courseProfessorMappingByCourseList = courseProfessorMappingService
				.findCourseProfessorMappingByCourse(professorDropDownListRequestDTO.getCourseId());
	...
	public void saveSectionSchedule(SectionScheduleSaveDTO sectionScheduleSaveDTO) {
		...
		try {
			...
			List<SectionSchedule> sectionScheduleList = sectionScheduleRepository
	...
	public SuggestionsResponseDTO getSuggestions(SuggestionRequestDTO suggestionsRequestDTO) {
		...
		if (suggestionsRequestDTO.getProfessorId() != null) {
			if (!fetchProfessorConflctsData.isEmpty() && fetchProfessorConflctsData.size() > 0) {
				for (SectionSchedule sectionScheduleForProfessor : fetchProfessorConflctsData) {
					...
					CourseSemesterMapping byCourseSemesterMappingId = courseSemesterMapping
							.findByCourseSemesterMappingId(section.getCourseSemesterMappingId());
					CourseDetails courseDetails = coursedetailsService
							.findCourseDetailsByCourseId(byCourseSemesterMappingId.getCourseId());
					String courseName = courseDetails.getCourseName();

					String finalAnswer = "professor " + professorName + " is occupied  in " + weekDay + " from "
							+ startTime + " to " + endTime + " with " + courseName + " section no  " + sectionNo;
		...
		if (suggestionsRequestDTO.getTimeSlots() != null && suggestionsRequestDTO.getTimeSlots().length > 0) {
			if (fetchCourseConflctsData != null && !fetchCourseConflctsData.isEmpty()
					&& fetchCourseConflctsData.size() > 0) {
				for (SectionSchedule sectionScheduleForCourse : fetchCourseConflctsData) {
					...
					String finalAnswer = sectionNo + " is scheduled on " + weekDay + " from " + startTime + " to "
							+ endTime;
		...
		if (suggestionsRequestDTO.getRoomId() != null) {
			if (fetchRoomConflctsData != null && !fetchRoomConflctsData.isEmpty() && fetchRoomConflctsData.size() > 0) {
				for (SectionSchedule sectionScheduleForRoom : fetchRoomConflctsData) {
					...
					String finalAnswer = room.getRoomName() + " is occupied on " + weekDay + " from " + startTime
							+ " to " + endTime + " for course " + courseName;
	...
	public List<SectionSchedule> fetchCourseConflctsData(SuggestionRequestDTO suggestionsRequest) {
		...
		for (TimeSlotsDTO timeSlotsDTO : timeSlots) {
			...
			if (!filteredCourseSemesterMappingList.isEmpty()) {
				...
				List<Section> filterSections = sectionService
						.findAll().stream().filter(section -> section
								.getCourseSemesterMappingId() == courseSemesterMapping.getCourseSemesterMappingId())
						.collect(Collectors.toList());
				if (!filterSections.isEmpty()) {
										...
										if (day.equalsIgnoreCase(WeekEnum.getWeekByValue(sr.getWeekDay()))) {
											...
											boolean isStartTimeInRange = (timeSlotsDTO.getStartTime()
													...
													|| (timeSlotsDTO.getStartTime().isAfter(sr.getStartTime())
															&& timeSlotsDTO.getStartTime().isBefore(sr.getEndTime())));
											...
											boolean isEndTimeInRange = (timeSlotsDTO.getEndTime()
													...
													|| (timeSlotsDTO.getEndTime().isAfter(sr.getStartTime())
															&& timeSlotsDTO.getEndTime().isBefore(sr.getEndTime())));

											// If both start and end times of timeSlotsDTO are within sr's time range,
											// return true
											return isStartTimeInRange && isEndTimeInRange;
	...
	public SectionScheduleSaveDTO getSectionSceduleBySectionSceduleId(Long sectionId) {
		...
		try {
			...
			for (Map.Entry<String, List<GetTimeSlotIterativeDTO>> entry : map.entrySet()) {
				...
				dto.setDays(weekstr);
				// LocalTime startTime = LocalTime.parse(getTimeSlotIterativeDTO.getStartTime(),
				// DateTimeFormatter.ofPattern("HH:mm"));
				// LocalTime endTime = LocalTime.parse(getTimeSlotIterativeDTO.getEndTime(),
				// DateTimeFormatter.ofPattern("HH:mm"));
