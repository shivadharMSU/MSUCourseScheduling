...
public class SectionServiceImpl implements SectionService {
	...
	public CrossSectionDropDownList getCrossSectionList(Integer semId, Long courseId) {
		...
		CourseSemesterMapping courseSemesterMapping = courseSemesterMappingRepository.findBySemesterIdAndCourseId(semId,
				courseId);
		...
		List<Section> sectionsList = findAll().stream().filter(
				section -> section.getCourseSemesterMappingId() != courseSemesterMapping.getCourseSemesterMappingId())
		...
		for (Section section : sectionsList) {
			...
			CourseSemesterMapping courseSemesterMappingObj = courseSemesterMappingRepository
					.findByCourseSemesterMappingId(section.getCourseSemesterMappingId());
			CourseDetails courseDetail = courseDetailsService
					.findCourseDetailsByCourseId(courseSemesterMappingObj.getCourseId());
