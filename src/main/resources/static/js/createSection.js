document.addEventListener("DOMContentLoaded", function () {
  ...
  function addTimeSlotTwo(slot = {}) {
    ...
    newTimeSlotDiv.innerHTML = `
                  ...
                  .map(
                    (day) =>
                      `<button class="btn btn-outline-primary ${
                        selectedDays.includes(day.toUpperCase())
                          ? "active btn-primary"
                          : ""
            ...
            <div class="col">
                <input type="time" class="form-control" id="startTime${index}" value="${
      slot.startTime || ""
    }" />
            </div>
            <div class="col">
                <input type="time" class="form-control" id="endTime${index}" value="${
      slot.endTime || ""
    }" />
            </div>
            <div class="col-auto">
    ...
    newTimeSlotDiv
      .querySelector(".remove-time-slot")
  ...
  function getTimeSlots() {
    const timeSlots = [];
    document
      .querySelectorAll("#timeSlotsContainer .row")
      .forEach((timeSlot) => {
        const dayButtons = timeSlot.querySelectorAll(
        ...
        if (dayButtons.length > 0 && startTimeInput && endTimeInput) {
          const days = Array.from(dayButtons).map(
  ...
  function fetchRooms() {
    return fetch("http://localhost:8080/getClassListForDropDown", {
      ...
      .then((response) =>
        response.ok ? response.json() : Promise.reject("Failed to fetch rooms")
      )
      .then((data) => {
  ...
  function fetchProfessors() {
    return fetch("http://localhost:8080/getProfessorListForDropDown", {
      ...
      .then((response) =>
        response.ok
          ? response.json()
          : Promise.reject("Failed to fetch professors")
      )
      .then((data) => {
    ...
    .addEventListener("change", function () {
      ...
      if (sectionId) {
        fetch(`http://localhost:8080/getSectionSchedule/${sectionId}`)
          ...
          .catch((error) =>
            console.error("Error fetching cross section schedule:", error)
          );
      }
    });
  ...
  function fetchCrossSectionList(semId, courseId) {
    return fetch(
      `http://localhost:8080/getCrossSectionDropDownList/${semId}/${courseId}`,
      {
        method: "GET",
      }
    )
