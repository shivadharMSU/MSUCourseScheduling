    ...
    classroomSelect.addEventListener("change", function () {
      ...
      if (selectedClassroomId) {
        const filteredClassrooms = classrooms.filter(
          (classroom) => classroom.roomId.toString() === selectedClassroomId
        );
        displayClassrooms(filteredClassrooms);
      } else {
  ...
  classrooms.forEach((classroom) => {
    if (classroom.schedule && classroom.schedule.length > 0) {
      classroom.schedule.forEach((schedule) => {
        ...
        row.insertCell().textContent = `${schedule.courseName || "N/A"}_${
          schedule.sectionNo || "N/A"
        }`;
      });
    } else {
      const row = reportContent.insertRow();
...
document.getElementById("export-pdf").addEventListener("click", function () {
  ...
  doc.autoTable({
    ...
    body: Array.from(document.querySelectorAll("#report-content tr")).map(
