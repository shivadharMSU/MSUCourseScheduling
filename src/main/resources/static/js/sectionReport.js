...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("semesterDropdown")
  ...
  document
    .getElementById("professorDropdown")
    .addEventListener("change", filterTable);
  document
    .getElementById("classRoomDropdown")
    .addEventListener("change", filterTable);
});
...
function clearTable() {
  document.getElementById("reportTableBody").innerHTML =
    '<tr><td colspan="5" class="text-center">Select a semester to view the report</td></tr>';
}
...
function filterTable() {
  ...
  const filteredData = globalReportData.filter((row) => {
    ...
    const matchesProfessor =
      selectedProfessor === "" || row.prodessorName === selectedProfessor;
